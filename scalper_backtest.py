...
=====================================================
Usage:
    python scalper_backtest.py --days 180
    python scalper_backtest.py --days 90
    python scalper_backtest.py --days 365
    python scalper_backtest.py --days 60 --demo
    python scalper_backtest.py --days 90 --symbol EURUSDm

All configuration and credentials live in config.py.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Forex Scalping Backtest Engine")
    parser.add_argument("--days",   type=int, default=180,
                        help="Number of past days to backtest (default: 180)")
    parser.add_argument("--demo",   action="store_true",
                        help="Use synthetic data instead of MT5")
    parser.add_argument("--symbol", type=str, default=None,
                        help="Override symbol (e.g. --symbol EURUSDm)")
...
def prepare_indicators(df: pd.DataFrame, cfg: BacktestConfig) -> pd.DataFrame:
    ...
    df["date"]     = df.index.date   # used for daily trade counter
...
def run_backtest(df: pd.DataFrame, cfg: BacktestConfig) -> BacktestResult:
    ...
    trade_id          = 0
    open_trade        = {}
    daily_trade_count = {}   # date → count of trades taken that day
    ...
    for i in range(1, n - 1):
        ...
        bar_date = g(row, "date")
        trades_today = daily_trade_count.get(bar_date, 0)
        if trades_today >= cfg.MAX_TRADES_PER_DAY:
            continue

        ema_f = g(row, "ema_fast")
        ema_s = g(row, "ema_slow")
        rsi_v = g(row, "rsi")
        atr_v = g(row, "atr")
        is_b  = bool(g(row, "is_bull"))
        is_br = bool(g(row, "is_bear"))

        # ── Gate 2: EMA separation — avoid choppy/ranging markets ─────────────
        ema_separation = abs(ema_f - ema_s)
        if ema_separation < cfg.EMA_MIN_SEPARATION:
            continue

        # ── Gate 3: ATR minimum — avoid dead/low-volatility periods ──────────
        if atr_v < cfg.ATR_MIN_VALUE:
            continue

        # ── Gate 4: RSI momentum + EMA trend + strong candle body ─────────────
        signal = None
        if ema_f > ema_s and cfg.RSI_LONG_MIN <= rsi_v <= cfg.RSI_LONG_MAX and is_b:
            signal = "BUY"
        elif ema_f < ema_s and cfg.RSI_SHORT_MIN <= rsi_v <= cfg.RSI_SHORT_MAX and is_br:
            signal = "SELL"
        ...
        trade_id += 1
        in_trade  = True
        daily_trade_count[bar_date] = trades_today + 1
...
def print_report(stats: dict, cfg: BacktestConfig):
    ...
    print("=" * W)
    print("\n  FILTERS APPLIED")
    row("RSI long zone",       f"{cfg.RSI_LONG_MIN} – {cfg.RSI_LONG_MAX}")
    row("RSI short zone",      f"{cfg.RSI_SHORT_MIN} – {cfg.RSI_SHORT_MAX}")
    row("Min candle body",     f"{int(cfg.CANDLE_BODY_PCT*100)}%")
    row("Min EMA separation",  f"{cfg.EMA_MIN_SEPARATION*10000:.0f} pips")
    row("Min ATR",             f"{cfg.ATR_MIN_VALUE*10000:.0f} pips")
...
def plot_results(result: BacktestResult, stats: dict, cfg: BacktestConfig):
    ...
    C_GREEN="#1D9E75"; C_RED="#D85A30"; C_BLUE="#378ADD"
    C_GRAY="#888780"; C_BG="#FAFAF8"; C_TEXT="#2C2C2A"
    C_MUTED="#5F5E5A"; C_LINE="#E8E6DF"
    plt.rcParams.update({
        "font.family":"DejaVu Sans","axes.facecolor":C_BG,
        "figure.facecolor":"#FFFFFF","axes.edgecolor":C_LINE,
        "axes.labelcolor":C_MUTED,"xtick.color":C_MUTED,"ytick.color":C_MUTED,
        "grid.color":C_LINE,"grid.linewidth":0.5,"axes.grid":True,
        "axes.spines.top":False,"axes.spines.right":False,
    })
    fig = plt.figure(figsize=(16, 14))
    fig.suptitle(
        f"Scalping Backtest  ·  {cfg.SYMBOL}  ·  "
    ...
    ax1.plot(times, equity, color=C_BLUE, linewidth=1.2, zorder=3)
    ax1.fill_between(times, cfg.INITIAL_BALANCE, equity,
                     where=[e >= cfg.INITIAL_BALANCE for e in equity], color=C_GREEN, alpha=0.12)
    ax1.fill_between(times, cfg.INITIAL_BALANCE, equity,
                     where=[e < cfg.INITIAL_BALANCE for e in equity], color=C_RED, alpha=0.15)
    ax1.axhline(cfg.INITIAL_BALANCE, color=C_GRAY, linewidth=0.8, linestyle="--", alpha=0.6)
    for t in trades:
        ax1.scatter(t.close_time, t.equity_after,
                    color=C_GREEN if t.outcome=="WIN" else C_RED, s=8, zorder=4, alpha=0.4)
    ...
    ax1.annotate(f"${final:,.0f}  ({'+' if pct>=0 else ''}{pct:.1f}%)",
                 ...
                 color=C_GREEN if final>=cfg.INITIAL_BALANCE else C_RED)
    ax1.set_ylabel("Equity ($)"); ax1.set_title("Equity curve", fontsize=10, color=C_MUTED, pad=6)
    ax1.yaxis.set_major_formatter(plt.FuncFormatter(lambda x,_:f"${x:,.0f}"))

    ax2=fig.add_subplot(gs[1,:]); eq_a=np.array(equity); peak=np.maximum.accumulate(eq_a)
    dd=(eq_a-peak)/peak*100
    ax2.fill_between(times,dd,0,color=C_RED,alpha=0.35,zorder=2)
    ax2.plot(times,dd,color=C_RED,linewidth=0.7,zorder=3)
    ax2.axhline(0,color=C_GRAY,linewidth=0.5); ax2.set_ylabel("Drawdown (%)")
    ax2.set_title("Drawdown from equity peak",fontsize=10,color=C_MUTED,pad=6)
    ...
    if monthly:
        ...
        ax3.set_title("Monthly returns (%)",fontsize=10,color=C_MUTED,pad=6); ax3.grid(False)
        for y in range(len(years)):
            for m in range(12):
                v=hdata[y,m]
                if not np.isnan(v):
                    ax3.text(m,y,f"{v:.1f}",ha="center",va="center",fontsize=7,
                             color="white" if abs(v)>vmax*0.55 else C_TEXT)
    ...
    if av:
        ...
        if wp: ax4.hist(wp,bins=bins,color=C_GREEN,alpha=0.65,label=f"Wins ({len(wp)})")
        if lp: ax4.hist(lp,bins=bins,color=C_RED,alpha=0.65,label=f"Losses ({len(lp)})")
        if wp: ax4.axvline(np.mean(wp),color=C_GREEN,linewidth=1.2,linestyle="--",
                           label=f"Avg win ${np.mean(wp):.0f}")
        if lp: ax4.axvline(np.mean(lp),color=C_RED,linewidth=1.2,linestyle="--",
                           label=f"Avg loss ${np.mean(lp):.0f}")
    ax4.set_xlabel("P&L ($)"); ax4.set_ylabel("Number of trades")
    ax4.set_title("P&L distribution",fontsize=10,color=C_MUTED,pad=6)
    ...
    fig.text(0.5,0.012,footer,ha="center",fontsize=9,color=C_MUTED,
             bbox=dict(boxstyle="round,pad=0.4",facecolor=C_LINE,alpha=0.5,edgecolor="none"))
...
def main():
    ...
    print("=" * 56)
    print(f"  FOREX SCALPING BACKTEST ENGINE")
    ...
    df_raw = get_data(cfg, use_demo=args.demo)
    ...
    if not result.trades:
        print("\n  No trades generated.")
        print("  Filters may be too tight — try loosening EMA_MIN_SEPARATION or ATR_MIN_VALUE in config.py")
        return
